...
// Endpoint per RESET complet de fantasy_scores i matches (PERILLÓS!)
router.post('/reset-fantasy', async (req, res) => {
  try {
    const { pool } = await import('../config/db.js');
    
    // Esborrar totes les dades fantasy i partits
    await pool.query('DELETE FROM fantasy_scores');
    await pool.query('DELETE FROM matches WHERE id NOT IN (SELECT DISTINCT match_id FROM bets)');
    
    // Reinicialitzar amb dades correctes
    const teams = [
      'Jaume Creixell U.E.',
      "L'ESQUADRA VILAS...",
      'CE FerranitoPito',
    ...
    // Actualitzar partits (només si no tenen apostes)
    const matches = await matchQueries.getAll();
    const matchesToUpdate = matches.filter(m => m.team2 === 'LINYOLETS E.C.');
    
    for (const match of matchesToUpdate) {
      await pool.query('UPDATE matches SET team2 = $1 WHERE id = $2', ['Nottingham_Pressa', match.id]);
    }

    res.json({
      success: true,
      message: 'Dades fantasy i partits actualitzats correctament',
      updatedTeams: teams.length,
      updatedMatches: matchesToUpdate.length
    });
  } catch (error) {
    console.error('Error resetejant fantasy:', error);
    res.status(500).json({
      success: false,
      error: 'Error resetejant dades fantasy',
      details: error.message
    });
  }
});
