...
// Si hi ha DATABASE_URL (Render), usa el núvol. Si no, usa localhost.
...
export async function initDatabase() {
  ...
  } catch (error) { console.error('❌ Error DB:', error); throw error; }
}

export const userQueries = {
  create: async (u, p, a) => { const r = await query('INSERT INTO users (username, password, is_admin) VALUES (, , ) RETURNING id', [u, p, a]); return { lastInsertRowid: r.rows[0].id }; },
  findByUsername: async (u) => get('SELECT * FROM users WHERE username = ', [u]),
  findById: async (id) => get('SELECT * FROM users WHERE id = ', [id]),
  updateCoins: async (c, id) => { const r = await query('UPDATE users SET coins =  WHERE id = ', [c, id]); return { changes: r.rowCount }; },
  getAll: async () => all('SELECT id, username, coins, is_admin, created_at FROM users ORDER BY coins DESC')
};

export const matchQueries = {
  create: async (t1, t2, r, b) => { const res = await query('INSERT INTO matches (team1, team2, round, betting_closes_at) VALUES (, , , ) RETURNING id', [t1, t2, r, b]); return { lastInsertRowid: res.rows[0].id }; },
  ...
  updateStatus: async (s, id) => { const r = await query('UPDATE matches SET status =  WHERE id = ', [s, id]); return { changes: r.rowCount }; },
  setResult: async (s1, s2, id) => { const r = await query("UPDATE matches SET score_team1 = , score_team2 = , status = 'finished', result_date = CURRENT_TIMESTAMP WHERE id = ", [s1, s2, id]); return { changes: r.rowCount }; },
  delete: async (id) => { const r = await query('DELETE FROM matches WHERE id = ', [id]); return { changes: r.rowCount }; }
};

export const betQueries = {
  create: async (uid, mid, bt, s, a, o, pr) => { const r = await query('INSERT INTO bets (user_id, match_id, bet_type, selection, amount, odds, potential_return) VALUES (, , , , , , ) RETURNING id', [uid, mid, bt, s, a, o, pr]); return { lastInsertRowid: r.rows[0].id }; },
  findById: async (id) => get('SELECT * FROM bets WHERE id = ', [id]),
  getByUser: async (uid) => all('SELECT b.*, m.team1, m.team2, m.round, m.status as match_status, m.score_team1, m.score_team2 FROM bets b JOIN matches m ON b.match_id = m.id WHERE b.user_id =  ORDER BY b.created_at DESC', [uid]),
  getByMatch: async (mid) => all('SELECT b.*, u.username FROM bets b JOIN users u ON b.user_id = u.id WHERE b.match_id = ', [mid]),
  updateStatus: async (s, r, id) => { const res = await query('UPDATE bets SET status = , result =  WHERE id = ', [s, r, id]); return { changes: res.rowCount }; },
  ...
  delete: async (id) => { const r = await query('DELETE FROM bets WHERE id = ', [id]); return { changes: r.rowCount }; }
};

export const parlayQueries = {
  create: async (uid, a, to, pr) => { const r = await query('INSERT INTO parlay_bets (user_id, amount, total_odds, potential_return) VALUES (, , , ) RETURNING id', [uid, a, to, pr]); return { lastInsertRowid: r.rows[0].id }; },
  addItem: async (pid, bid) => { const r = await query('INSERT INTO parlay_bet_items (parlay_bet_id, bet_id) VALUES (, ) RETURNING id', [pid, bid]); return { lastInsertRowid: r.rows[0].id }; },
  getByUser: async (uid) => all('SELECT * FROM parlay_bets WHERE user_id =  ORDER BY created_at DESC', [uid]),
  getItems: async (pid) => all('SELECT b.*, m.team1, m.team2 FROM parlay_bet_items pbi JOIN bets b ON pbi.bet_id = b.id JOIN matches m ON b.match_id = m.id WHERE pbi.parlay_bet_id = ', [pid]),
  findById: async (id) => get('SELECT * FROM parlay_bets WHERE id = ', [id]),
  updateStatus: async (s, r, id) => { const res = await query('UPDATE parlay_bets SET status = , result =  WHERE id = ', [s, r, id]); return { changes: res.rowCount }; },
  delete: async (id) => { const r = await query('DELETE FROM parlay_bets WHERE id = ', [id]); return { changes: r.rowCount }; }
};

export const transactionQueries = {
  create: async (uid, a, t, d) => { const r = await query('INSERT INTO transactions (user_id, amount, type, description) VALUES (, , , ) RETURNING id', [uid, a, t, d]); return { lastInsertRowid: r.rows[0].id }; },
  getByUser: async (uid) => all('SELECT * FROM transactions WHERE user_id =  ORDER BY created_at DESC LIMIT 50', [uid])
};

export const fantasyQueries = {
  addScore: async (t, m, p) => { const r = await query('INSERT INTO fantasy_scores (team, matchday, points) VALUES (, , ) ON CONFLICT (team, matchday) DO UPDATE SET points =  RETURNING id', [t, m, p]); return { lastInsertRowid: r.rows[0].id }; },
  getByMatchday: async (m) => all('SELECT * FROM fantasy_scores WHERE matchday =  ORDER BY points DESC', [m]),
  ...
  getTeamHistory: async (t) => all('SELECT * FROM fantasy_scores WHERE team =  ORDER BY matchday ASC', [t])
};
